...
const Cart = ({location, getLocation}) => {
  ...
  const [name, setName] = React.useState('Guest User')
              ...
              {cartItem.map((item, index) => {
                return <div key={index} className='bg-gray-100 p-5 rounded-md flex items-center justify-between mt-3 w-full'>
                  <div className='flex items-center gap-4'>
                    <img src={item.images?.[0] || 'https://via.placeholder.com/80?text=No+Image'} alt={item.title} className='w-20 h-20 rounded-md object-cover' />
                    <div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 md:gap-20'>
              <div className='bg-gray-100 rounded-md p-7 mt-4 space-y-2'>
                ...
                <div className='flex flex-col space-y-1'>
                  ...
                  <input id="fullName" type="text" placeholder='Enter your name' className='p-2 rounded-md' value={name} onChange={(e) => setName(e.target.value)} />
                </div>
                <div className='flex flex-col space-y-1'>
                  <label htmlFor="address">Address</label>
                  <input id="address" type="text" placeholder='Enter your address' className='p-2 rounded-md bg-gray-50 cursor-not-allowed' value={location?.county || ''} readOnly />
                </div>
                <div className='flex w-full gap-5'>
                  <div className='flex flex-col space-y-1 w-full'>
                    <label htmlFor="state">State</label>
                    <input id="state" type="text" placeholder='Enter your state' className='p-2 rounded-md w-full bg-gray-50 cursor-not-allowed' value={location?.state || ''} readOnly />
                  </div>
                  <div className='flex flex-col space-y-1 w-full'>
                    <label htmlFor="postcode">PostCode</label>
                    <input id="postcode" type="text" placeholder='Enter your postcode' className='p-2 rounded-md w-full bg-gray-50 cursor-not-allowed' value={location?.postcode || ''} readOnly />
                  </div>
                </div>
                <div className='flex w-full gap-5'>
                  <div className='flex flex-col space-y-1 w-full'>
                    <label htmlFor="country">Country</label>
                    <input id="country" type="text" placeholder='Enter your country' className='p-2 rounded-md w-full bg-gray-50 cursor-not-allowed' value={location?.country || ''} readOnly />
                  </div>
                  <div className='flex flex-col space-y-1 w-full'>
                    <label htmlFor="phone">Phone No</label>
                    <input id="phone" type="tel" placeholder='Enter your Number' className='p-2 rounded-md w-full' />
                  </div>
                </div>
